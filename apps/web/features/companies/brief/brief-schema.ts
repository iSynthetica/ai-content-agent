...
// Схема — фабрика, а не константа: повідомлення валідації йдуть через t() (UI-мова), а t()
// доступний лише в компоненті (useT()/useLanguage()). BriefForm будує схему через
// useMemo(() => makeBriefSchema(t), [t]), тож вона перебудовується при зміні мови.
export function makeBriefSchema(t: (source: string) => string) {
  return z.object({
    // company core
    name: z.string().min(1, t("Вкажіть назву компанії")),
    websiteUrl: z
      .string()
      .trim()
      .url(t("Некоректний URL (приклад: https://forteq.systems)"))
      .or(z.literal(""))
      .optional(),
